(()=> {
  onload = (event) => { 
//Variables entrées par l'administrateur;
let canvasGames = document.getElementById('canvasGames');
if(canvasGames != null) {
let cWidth = canvasGames.dataset.cwidth; 
let cHeight = canvasGames.dataset.cheight; 
let textStart = canvasGames.dataset.textstart;
let textGameover = canvasGames.dataset.textgameover;
let backColor = canvasGames.dataset.colorback;
...
var Paddle = {
	new: function (side) {
		return {
			width: 18,
			height: cHeight? (cHeight*0.13) : 70,
...
function getCookieValue() {
  ...
  .split('; ')
  .find((row) => row.startsWith('Best_Score_Pong='))
  ?.split('=')[1];
...
var Game = {
	initialize: function () {
		...
		this.canvas.width = cWidth && cWidth > 0 ? cWidth :1400;
		this.canvas.height = cHeight && cHeight > 0 ? cHeight :1000;
		...
		this.color = backColor && backColor != "" ? backColor:'#2c3e50';
	...
	menu: function () {
		...
		this.context.fillText(textStart && textStart != "" ? textStart :'Press any key to begin',
			this.canvas.width / 2,
